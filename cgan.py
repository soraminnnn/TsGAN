...
from model.discriminator.netD import CGAN_Discriminator

parser = argparse.ArgumentParser()
parser.add_argument("--data", type=str, default="data/alibaba_machine/train.npy",help="path to load the training data")
parser.add_argument("--label", type=str, default="data/alibaba_machine/label.npy",help="path to load the labels of the training data")

parser.add_argument("--use_GPU", type=bool, default=True, help="whether use GPU")
parser.add_argument("--latent_dim", type=int, default=100, help="dimensionality of the latent space")
parser.add_argument("--models_dir", type=str, default="models_saved",help='path to save the trained models.')
...
parser.add_argument("--g_num_layers", type=int, default=8, help="number of layers of the generator")
parser.add_argument("--g_num_filters", type=int, default=25, help="number of filters of the generator")
parser.add_argument("--g_kernel_size", type=int, default=7, help="kernel size of convolution for the generator")
###discriminator parameters###
parser.add_argument("--d_num_layers", type=int, default=4, help="number of layers of the discriminator")
parser.add_argument("--d_downsample", type=bool, nargs='+', default=[True]*4, help="whether use average pool in each layer of the discriminator")
...
def train():
    ...
    n_classes = np.max(label)-np.min(label)+1

    fix_z = torch.randn(5, args.latent_dim).to(device)
    z_label = torch.LongTensor(np.random.randint(0,n_classes,5))
    fix_label = z_label.to(device)
    ...
    net_g = Generator(latent_dim=args.latent_dim, seq_len=series_len, input_dim=1, output_dim=series_dim, num_channels=[args.g_num_filters]*args.g_num_layers,kernel_size=args.g_kernel_size,n_classes=n_classes).to(device)
    net_d = CGAN_Discriminator(in_channels=series_dim,num_layers=args.d_num_layers,downsample=args.d_downsample,num_classes=n_classes).to(device)
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        for i, (real_data, label) in enumerate(data_loader):
            ...
            outputs = net_d(real_data,label)
            d_x = outputs
            d_loss_real = criterion(outputs, true_lable)
            # fake_loss for the discriminator
            z = torch.randn(latent_shape).to(device)
            z_label = torch.LongTensor(np.random.randint(0, n_classes, true_batch_size)).to(device)
            fake_data = net_g(z,z_label).view(true_batch_size,*output_shape)
            outputs = net_d(fake_data,z_label)
        ...
        if (epoch+1) % args.samples_iter == 0:
            net_g.eval()
            samples = net_g(fix_z,fix_label).cpu().detach().numpy()
            ...
            for n in range(samples.shape[0]):
                ...
                plt.title('class %s'%fix_label[n].item())
